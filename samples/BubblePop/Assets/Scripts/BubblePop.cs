...
public class BubblePop : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		"knetik-bubblepop-test"								// (required) Customer ID from the Splyt team.  If you don't have one, contact them.
		...
		//,deviceInfo: Splyt.EntityInfo.createDeviceInfo().setProperty("screenwidth", 1024)	// (optional) Only generally needed if device properties are sent at startup
        ...
        //,host: "http://localhost"								// (don't use) This is for Splyt developers only
		);
	...
	//
	// Sent to all game objects when the player pauses/resumes. We need to pause/resume splyt's core as well
	//
	...
	{
		Splyt.Tuning.refresh(delegate(Splyt.Error refreshError) {
			if(Splyt.Error.Success == refreshError)
				Debug.Log("onSplytRefreshComplete: " + refreshError.ToString());
			else
				Debug.LogError("onSplytRefreshComplete: " + refreshError.ToString());
	...
	private bool _BuyGame()
	{
		if(mMode == Mode.Pay)
		{
			...
			Splyt.Plugins.Purchase.Transaction()
	...
	private void _InitGame()
	{
		if(false == mInited)
		{
			...
			mGold = Splyt.Tuning.getVar("initialGoldAmount", 200);
			
			mInited = true;
		}	
		
		// Retrieve a value for a string variable, which can be dynamically set on the server
		mGreeting = Splyt.Tuning.getVar("greeting", "Hello Popper!");
		...
		mMode = Splyt.Tuning.getVar("gameMode", Mode.Pay);
		
		// Unity's Color struct is not an enum, so we take a different approach for tuning that
		string colorValue = Splyt.Tuning.getVar("bubbleColor", "cyan");
