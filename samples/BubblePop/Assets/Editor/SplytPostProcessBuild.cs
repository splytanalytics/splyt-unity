...
public class SplytPostProcessBuild
{
	[PostProcessBuild]
	public static void AddSplytFramework(BuildTarget target, string path)
	{
		#if UNITY_IPHONE
		...
		string frameworkPath = Application.dataPath + "/Plugins/iOS/Splyt.framework";
		...
		// if the project already includes Splyt.framework, we don't need to do anything
		if(null != Array.Find(lines, delegate(string line) { return line.Contains("Splyt.framework"); }))
			return;

		Debug.Log("Splyt.framework - not found, adding to PBXPROJ");
			...
			foreach(string line in lines)
			{
				...
				if(section == "XCBuildConfiguration")
				{
					...
					if(inBuildSettings && line.Trim().StartsWith("}"))
					{
						if(!addedSearch)
						{
							Debug.Log("Splyt.framework - adding FULL framework search path");
					...
					{
						Debug.Log("Splyt.framework - adding framework search path");
						proj.WriteLine("                    \"\\\"$(SRCROOT)/" + Path.GetDirectoryName(frameworkPath) + "\\\"\",");
				...
				if(null != match && match.Success)
				{
					...
					{
					case "PBXBuildFile":
						proj.WriteLine("		3F474F21187DCB8E0047C5E2 /* Splyt.framework in Frameworks */ = {isa = PBXBuildFile; fileRef = 3F474F1F187DCB200047C5E2 /* Splyt.framework */; };");
						Debug.Log("Splyt.framework - adding build file");
						break;
						
					case "PBXFileReference":
						proj.WriteLine("		3F474F1F187DCB200047C5E2 /* Splyt.framework */ = {isa = PBXFileReference; lastKnownFileType = wrapper.framework; name = Splyt.framework; path = \"" + frameworkPath + "\"; sourceTree = \"<group>\"; };");
						Debug.Log("Splyt.framework - adding file reference");
						break;
					}
				}
				...
				case "PBXFrameworksBuildPhase":
					...
					{
						proj.WriteLine("				3F474F21187DCB8E0047C5E2 /* Splyt.framework in Frameworks */,");
						Debug.Log("Splyt.framework - adding to build phase");
					}
					break;
				case "PBXGroup":
					...
					{
						proj.WriteLine("				3F474F1F187DCB200047C5E2 /* Splyt.framework */,");
						Debug.Log("Splyt.framework - adding to frameworks group");
					}
					break;
				}
			}
		...
		Debug.Log("Splyt.framework - done adding framework to PBXPROJ");
		#endif
	}
}